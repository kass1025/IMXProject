using DIMS.Entities;
using DIMS.IRepositories;
using Microsoft.AspNetCore.Mvc;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace DIMS.Controllers
{
    public class GroupTypeController : Controller
    {
        private readonly IGroupTypeRepository _IgroupTypeRepository;
        public GroupTypeController(IGroupTypeRepository IgroupTypeRepository)
        {
            _IgroupTypeRepository = IgroupTypeRepository;
        }
        public async Task<ViewResult> Index(bool isSuccess = false, string DuplicateError = "")
        {
            if (DuplicateError != "")
            {
                ViewBag.DuplicateErrormsg = DuplicateError;
                ViewBag.DuplicateError = true;
            }
            else
            {
                ViewBag.DuplicateError = false;
            }
            ViewBag.isSuccess = isSuccess;
            ViewBag.GetAllGroupType = await _IgroupTypeRepository.GetAllGroupTypeAsync();

            return View();
        }

        [HttpPost]
        public async Task<IActionResult> AddNewGroupType(GroupType groupType )
        {
            if (ModelState.IsValid)
            {
                //Check uplication
                int count = await _IgroupTypeRepository.CheckGroupTypeDuplication(groupType);
                if (count > 0)
                {
                    ViewBag.DuplicateError = "Group type name Already  Exists!!";
                    return RedirectToAction(nameof(Index), new { isSuccess = true, DuplicateError = ViewBag.DuplicateError });
                }
                else
                {
                    var id = await _IgroupTypeRepository.AddNewGroupTypeAsync(groupType);
                    if (id > 0)
                    {
                        return RedirectToAction(nameof(Index), new { isSuccess = true });
                    }
                    else
                    {
                        ModelState.AddModelError("", "Incorrect Data");
                        ModelState.Clear();
                        return View(groupType);
                    }
                }
            }
            ModelState.Clear();
            return View();
        }
    }
}
