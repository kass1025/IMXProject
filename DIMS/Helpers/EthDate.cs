    ...
    public class EthiopianDate
        {
            ...
            public static string JulianToEthiopian(double jd) {
			var c = Math.Floor(jd) + 0.5 - jdEpoch;
			var year = Math.Floor((c - Math.Floor(((c + 366) / 1461))) / 365) + 1;
			if (year <= 0) { year--; } // No year zero
            c = Math.Floor(jd) + 0.5 - EthiopianToJulian(Convert.ToInt32(year), 1, 1);
			var month = Math.Floor(c / 30) + 1;
			var day = c - (month - 1) * 30 + 1;
			return new EthiopianDate(Convert.ToInt32(year), Convert.ToInt32(month), Convert.ToInt32(day)).ToDateString();
		}
            /// <summary>
            /// Gregs to ethiopian.
            /// </summary>
            ...
            {
                return ((((0xaeb + (0x16d * (ethiopianYear - 1))) + ((int)Math.Floor((double)(((double)ethiopianYear) / 4.0)))) + (30 * (ethiopianMonth - 1))) + ethiopianDay)+1;
            }
            ...
            public static string GregorianToEthiopian(DateTime gregorianDate)
            {
              ...
              //  gregorianYear = gregorianDate.Year;
               ...
               // gregorianDay = gregorianDate.Day;
                //gregorian to JD
               var jd= GregorianToJulianDate(gregorianDate);
                var etDate = JulianToEthiopian(jd);
                return etDate;
                // FixedToEthiopian(GregorianToFixed(gregorianYear, gregorianMonth, gregorianDay), ref gregorianYear, ref gregorianMonth, ref gregorianDay);
            ...
            public static DateTime EthiopianToGregorian(string ethiopianDateDDMMYYYY)
            {
                ...
                var jd = EthiopianToJulian(year, month, day);
                var grgCal = GregorianFromJulian(jd);
                //FixedToGregorian(EthiopianToFixed(year, month, day), ref year, ref month, ref day);
                //DateTime dt = new DateTime(year, month, day);
                return grgCal;
            }
            public  static DateTime GregorianFromJulian(double jd)
            {
                // Jean Meeus algorithm, "Astronomical Algorithms", 1991
            ...
            public static bool IsValidEthiopianDate(string ethiopianDateDDMMYYYY)
            {
                try
                {
                     string[] dateParts = ethiopianDateDDMMYYYY.Split('/');
